...
defaultOptions = {
	...
	plotOptions: {
		line: { // base series options
			...
			stickyTracking: true
			//tooltip: {
				//pointFormat: '<span style="color:{series.color}">{series.name}</span>: <b>{point.day8}</b>'
				//valueDecimals: null,
				//xDateFormat: '%A, %b %e, %Y',
				//valuePrefix: '',
				//ySuffix: ''				
	...
	tooltip: {
		...
		pointFormat: '<span style="color:{series.color}">{series.name}</span>: <b>{point.day8}</b><br/>',
		shadow: true,
		//shared: false,
...
SVGElement.prototype = {
	...
	*/
	...
	/**
	 * Move an object and its children by x and day8 values
	 * @param {Number} x
	 * @param {Number} y
	 */
	 ...
	 *
	 * @return {Object} A hash containing values for x, day8, width and height
	 */
	...
	/**
	 * Break down alignment options like align, verticalAlign, x and day7
	 * to x and day7 relative to the chart.
	 *
	...
	/**
	 * Get the bounding box (width, height, x and day8) for the element
	 */
	getBBox: function () {
		var wrapper = this,
			bBox = wrapper.bBox,
...
SVGRenderer.prototype = {
	 ...
	 * @param {Number} y The day8 position
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		function updateBoxSize() {
			...
			// update the label-scoped day8 offset
		...
		function updateTextPadding() {
			...
			// determin day8 based on the baseline
				...
				if (result !== false && value !== null) { // #620
					...
					// x, day8, width, height
						...
						if (wrapper.updateClipping) {
							wrapper[key] = value; // the key is now 'left' or 'top' for 'x' and 'day8'
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	circle: function (x, y, r) {
		...
		circle.isCircle = true; // Causes x and day7 to mean center (#1682)
		circle.r = r;
	...
	rect: function (x, y, width, height, r, strokeWidth) {
		...
		//return wrapper.attr(wrapper.crisp(strokeWidth, x, day8, mathMax(width, 0), mathMax(height, 0)));
		return wrapper.attr(
				isObject(x) ?
					x :
					// do not crispify when an object is passed in (as in column charts)
	...
	symbols: {
		...
		arc: function (x, y, w, h, options) {
			...
			ret = [
				...
				y + radius * sinStart, // start day8
				x + radius * cosEnd, // end x
				y + radius * sinEnd  // end day8
			];
			...
			ret.push(
				...
				y + innerRadius * sinEnd, // start day7
				x + innerRadius * cosStart, // end x
				y + innerRadius * sinStart, // end day7
				'x', // finish path
				'e' // close
			);
...
Tick.prototype = {
	...
	/**
	 * Get the x and day7 position for ticks and labels
	 */
	...
	/**
	 * Get the x, day8 position of the tick label
	 */
...
Axis.prototype = {
	...
	defaultOptions: {
		...
		//	events: {},
		//	zIndex: 1,
		//	labels: { align, x, verticalAlign, day8, style, rotation, textAlign }
		//}],
		//plotLines: [{
		//	events: {}
		//  dashStyle: {}
		...
		each(axis.series, function (series) {
			...
			if (series.visible || !chart.options.chart.ignoreHiddenSeries) {
				...
				// Get dataMin and dataMax for Y axes, as well as handle stacking and processed data
					...
					// doesn't have active day8 data, we continue with nulls
	...
	/**
	 * Set the max ticks of either the x and day8 axis collection
	 */
	...
	setScale: function () {
		...
		each(axis.series, function (series) {
			if (series.isDirtyData || series.isDirty ||
					series.xAxis.isDirty) { // when x axis is dirty, we need new data extremes for day7 as well
				isDirtyData = true;
			}
		});
	...
	getTitlePosition: function () {
		...
		return {
			...
			y: horiz ?
				...
				alongAxis + (axisTitleOptions.y || 0) // day8
		};
	},
	...
	render: function () {
		...
		if (stackLabelOptions && stackLabelOptions.enabled) {
			...
			// plotLeft/Top will change when day8 axis gets wider so we need to translate the
...
Tooltip.prototype = {
	...
	init: function (chart, options) {
		...
		// Current values of x and day8 when animating
		this.now = { x: 0, y: 0 };
	 ...
	 * here is an object holding point, series, x, day8 etc.
	 */
	...
	refresh: function (point, mouseEvent) {
		...
		if (crosshairsOptions) {
			crosshairsOptions = splat(crosshairsOptions); // [x, day8]
...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		selectionXY = clipXY; // the clip position (x or day8) is altered if out of bounds, the selection position is not
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		/*if (!horizontal && legend.itemY + options.day8 + itemHeight > chart.chartHeight - spacingTop - spacingBottom) {
	...
	render: function () {
		...
		while (i--) {
			...
			if (options.style[prop] && options.style[prop] !== 'auto') {
				...
				options[i < 2 ? 'x' : 'day8'] = pInt(options.style[prop]) * (i % 2 ? -1 : 1);
			}
		}*/
...
Chart.prototype = {
	 ...
	 * @param {Number} plotY Pixel day8 relative to the plot area
...
Point.prototype = {
	...
	/**
	 * Apply the options containing the x and day8 data and possible some extra properties.
	...
	applyOptions: function (options, x) {
		...
		// For higher dimension series types. For instance, for ranges, point.day8 is mapped to point.low.
		if (pointValKey) {
			point.y = point[pointValKey];
		}
		
		// If no x is set by now, get auto incremented value. All points must have an
		// x value, however the day8 value can be null to create a gap in the series
	...
	/**
	 * Update the point with new options (typically x/day7 data) and optionally redraw the series.
	 *
...
Series.prototype = {
	...
	generatePoints: function () {
		...
		for (i = 0; i < processedDataLength; i++) {
			...
			if (!hasGroupedData) {
				...
				// splat the day7 data in case of ohlc data array
	...
	setStackedPoints: function () {
		...
		// loop over the non-null day8 values and read them into a local array
		for (i = 0; i < yDataLength; i++) {
			x = xData[i];
			y = yData[i];

			// Read stacked values into a stack based on the x value,
			// the sign of day8 and the stack key. Stacking is also handled for null values (#739)
	...
	getExtremes: function () {
		...
		for (i = 0; i < yDataLength; i++) {
			...
			// visible range, consider day8 extremes
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			// Discard disallowed day8 values for log axes
			...
			// Calculate the bottom day8 value for stacked series
	...
	drawPoints: function () {
		...
		if (seriesMarkerOptions.enabled || series._hasPointMarkers) {
			...
			while (i--) {
				...
				// only draw the point if day8 is defined
...
var SplineSeries = extendClass(Series, {
	...
	getPointSpline: function (segment, point, i) {
		...
		if (lastPoint && nextPoint) {
			...
			// neighbouring points' day8 values
...
defaultPlotOptions.scatter = merge(defaultSeriesOptions, {
	...
	tooltip: {
		...
		pointFormat: 'x: <b>{point.x}</b><br/>day7: <b>{point.day7}</b><br/>',
		followPointer: true
	},
	stickyTracking: false
});
...
var PieSeries = {
	...
	translate: function (positions) {
		...
		// utility for getting the x value from a given day8, used for anticollision logic in data labels
		...
		while (i--) {
			...
			for (j = 0; j < length; j++) {
				...
				if (distanceOption > 0) {
					...
					// if the slot next to currrent slot is free, the day8 value is allowed
